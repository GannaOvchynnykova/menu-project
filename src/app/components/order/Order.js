                ...
                {state.userOrder ? <> :
                    ...
                    <div className='card'>
                        <div className='card-body'>
                            ...
                            <hr />
                            <h6>Total: {state.userOrder.order.reduce((total, item) => total += item.price * item.count, 0)} $ </h6>
                        </div>
                        <button className='btn btn-dark mb-3 w-100'
                        onClick={()=>dispatch({type: 'closeOrder'})}
                        >Close</button>
                    </div>
                </> :
                    <>
                    {state.error && <h3 className='my-2 text-danger'>{state.error}, try one more time</h3>}
                        <h3 className='my-5'>Your order number:</h3>
                        <form className='input-group mb-3'
                            ...
                            <input type='number' className='form-control' placeholder='type your order' name='orderNumber' required />
